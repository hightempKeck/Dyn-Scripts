...
#    print(f'layer = {layer_idx}, plate_scan_angle (pre-constraint) = {(math.degrees(plate_scan_angle))}, plate_angle_raw = {(math.degrees(plate_angle_raw))}')
    ...
    second_hatchc = dyn.HatchingParameters(hatch_spacing=0.12,scan_angle=plate_scan_angle,alternate_direction=True,hatch_length=1000,generation_origin=dyn.Vector2(0.12,0.12))  

    ctx.hatch_fragments(fragments=plate_2_frags,hatching_params=second_hatchc)
    ctx.hatch_fragments(fragments=plate_frags,hatching_params=second_hatchc)# hatching_params=plate_hatching)
