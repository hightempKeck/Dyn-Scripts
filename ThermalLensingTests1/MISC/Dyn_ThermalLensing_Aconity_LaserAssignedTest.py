...
cube = dyn.ops.load_part(path=r"C:\Users\alwoocay\Documents\Dyndrite\Thermal_Lensing_Plate.stl",
     auto_center=True,
     transform=None,
     translate_only=None,
     open_geometry=False,
...
# Seperate into 2 lasers

normal_melt_L1 = toolpather.create_build_style(
    cli_plus_params=dyn.CliPlusToolParameters.build({
        "laser_power": ("watt", "double", 285),
        "mark_speed": ("mm/s", "double", 1000),
        "laser_index": ("int", "double", 1),
    }))
normal_melt_L2 = toolpather.create_build_style(
    cli_plus_params=dyn.CliPlusToolParameters.build({
        "laser_power": ("watt", "double", 380),
        "mark_speed": ("mm/s", "double", 1000),
        "laser_index": ("int", "double", 2),
    }))
...
hatch_config_L1 = {core_seg0: normal_melt_L1}
perimeter_config_L1 = {core_seg0: (normal_melt_L1, [normal_melt_L1,normal_melt_L1])}

hatch_config_L2 = {core_seg0: normal_melt_L2}
perimeter_config_L2 = {core_seg0: (normal_melt_L2, [normal_melt_L2,normal_melt_L2])}

schema_L1 = toolpather.create_toolpath_schema(segmentation_strategy=segmentation,contour_strategy=contour_strat)
schema_L2 = toolpather.create_toolpath_schema(segmentation_strategy=segmentation,contour_strategy=contour_strat)

schema_L1.set_hatch_config(config=hatch_config_L1)
schema_L1.set_all_perimeter_configs(config=perimeter_config_L1)

schema_L2.set_hatch_config(config=hatch_config_L2)
schema_L2.set_all_perimeter_configs(config=perimeter_config_L2)
...
schema_L1.fill_default_hatch_generation(params=default_hatching)
schema_L2.fill_default_hatch_generation(params=default_hatching)
...
vp.apply_schema(geometry=cube,schema=schema_L1,region_segment_mapping=None)
for part in [cylinder_left, cylinder_right]:
    vp.apply_schema(geometry=part,schema=schema_L2,region_segment_mapping=None)
vp.finalize()
...
#vp.slice_all(writers=dyn.SlmWriter(out_file=output_path),on_slice=on_slice)


# Use Custom SLM File Writer to Slice
...
filepath = os.path.join(directory, "dyn_out_Test.ilt")
