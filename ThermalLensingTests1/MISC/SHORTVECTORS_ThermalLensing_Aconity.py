...
cube_mid_x = (cube_min_x + cube_max_x) / 2
cube_mid_y = (cube_min_y + cube_max_y) / 2
...
def create_rectangle_polygon(x_start, x_end, y_start, y_end):
    ...
    return dyn.Path([dyn.Vector2(a, b) for a, b in zip(x, y)])

# Four 50x50mm tiles in checkerboard write order (BL→TR→BR→TL)
cube_tile_polygons = [
    dyn.CuttingPolygon(outer_loop=create_rectangle_polygon(cube_min_x, cube_mid_x, cube_min_y, cube_mid_y)),  # BL
    dyn.CuttingPolygon(outer_loop=create_rectangle_polygon(cube_mid_x, cube_max_x, cube_mid_y, cube_max_y)),  # TR
    dyn.CuttingPolygon(outer_loop=create_rectangle_polygon(cube_mid_x, cube_max_x, cube_min_y, cube_mid_y)),  # BR
    dyn.CuttingPolygon(outer_loop=create_rectangle_polygon(cube_min_x, cube_mid_x, cube_mid_y, cube_max_y)),  # TL
]
...
def cb(ctx: dyn.LayerContext, writer: dyn.VectorWriter, layer_idx):
    ...
    # Hatch cube in 4 checkerboard tiles (BL→TR→BR→TL) to avoid long vectors
    cube_tile_frags_list = []
    for i, tile_poly in enumerate(cube_tile_polygons):
        tile = ctx.custom_cutting_tiles(cutting_polygons=[tile_poly])
        tile_frags = ctx.cut(collection=cube_frags, tiles=tile, cut_tag=f"cube_tile_{i}")
        ctx.hatch_fragments(fragments=tile_frags, hatching_params=cube_hatching)
        cube_tile_frags_list.append(tile_frags)

    writer.write_fragments(fragments=cylinder_left_frags)
    for tile_frags in cube_tile_frags_list:
        writer.write_fragments(fragments=tile_frags)
